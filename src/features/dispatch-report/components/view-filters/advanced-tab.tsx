...
import type {
  ...
  DispatchReportColumnId,
} from '@/features/dispatch-report/utils/report-filter-fns';
import { isAdvancedNumericColumn } from '@/features/dispatch-report/utils/report-filter-fns';
...
function getColumnLabel(column: Column<ReportFeatures, DispatchReportRow, unknown>) {
  return column.columnDef.meta?.filterLabel ?? column.id;
}

function getDefaultOperator(column: Column<ReportFeatures, DispatchReportRow, unknown>) {
  return isAdvancedNumericColumn(column.id) ? 'greaterThan' : 'contains';
}

function getOperatorOptions(
  column: Column<ReportFeatures, DispatchReportRow, unknown> | undefined,
) {
...
function createCondition(
  ...
  return {
    ...
    columnId: column.id as DispatchReportColumnId,
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Combine conditions with AND / OR logic. For example, party equals ABC AND total bags
              &gt; 10.
            </p>
          </div>
          <Button
