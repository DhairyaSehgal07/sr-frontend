...
const STORAGE_KEY = 'dispatch-report:column-preferences:v2';
...
export type DispatchReportColumnState = {
...
export function getDispatchReportColumnIds(
  columns: ColumnDef<ReportFeatures, Record<string, unknown>>[],
) {
  return columns.map(getColumnId);
}

export function getStoredDispatchReportColumnState(columnIds: string[]): DispatchReportColumnState {
...
export function hasStoredDispatchReportColumnState() {
  return parsePreferences(getStorage()?.getItem(STORAGE_KEY) ?? null) != null;
}

export function saveDispatchReportColumnState(
  columnIds: string[],
  columnVisibility: ColumnVisibilityState,
  columnOrder: ColumnOrderState,
) {
...
export function clearStoredDispatchReportColumnState() {
  const storage = getStorage();
  if (!storage) return false;
