...
import {
  ...
  isIntegerSummableExportColumn,
  isSummableExportColumn,
  sumExportColumn,
} from '@/features/dispatch-report/utils/export-cell-value';
import {
...
export const DISPATCH_REPORT_DOWNLOAD_EXCEL_MESSAGE = 'kf-dispatch-report-download-excel' as const;

export const DISPATCH_REPORT_DOWNLOAD_EXCEL_DONE_MESSAGE =
  'kf-dispatch-report-download-excel-done' as const;

export type PreviewDispatchReportOptions = {
...
export function buildDispatchReportPreviewHtml({
  table,
  coldStorageName,
...
}: PreviewDispatchReportOptions): string {
  ...
  const footerCells = visibleColumns
    .map((column, columnIndex) => {
      ...
      if (isSummableExportColumn(columnId)) {
        const total = sumExportColumn(filteredRows, columnId);
        const formatted = isIntegerSummableExportColumn(columnId)
  ...
  <body>
    ...
    <div class="toolbar">
      <button
        ...
        class="primary"
        onclick="downloadDispatchReportExcel()"
      >
        Download Excel
      </button>
    </div>
    <script>
      (function () {
        var DOWNLOAD_TYPE = ${JSON.stringify(DISPATCH_REPORT_DOWNLOAD_EXCEL_MESSAGE)};
        var DONE_TYPE = ${JSON.stringify(DISPATCH_REPORT_DOWNLOAD_EXCEL_DONE_MESSAGE)};
        ...
        window.downloadDispatchReportExcel = function () {
          if (!window.opener) {
...
export function openDispatchReportPreview(options: PreviewDispatchReportOptions): Window {
  const html = buildDispatchReportPreviewHtml(options);
