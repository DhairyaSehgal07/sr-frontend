...
export function sumBagSizeQuantityForRow(
  bagSize: readonly { size: string; quantityIssued: number }[],
  size: string,
): number {
  return bagSize.reduce((sum, item) => {
    if (item.size !== size) return sum;
    return sum + item.quantityIssued;
  }, 0);
}

export function sumBagSizeQuantity(
  rows: readonly { original: { bagSize: readonly { size: string; quantityIssued: number }[] } }[],
  size: string,
): number {
  return rows.reduce((sum, row) => sum + sumBagSizeQuantityForRow(row.original.bagSize, size), 0);
}
