...
export type ExportDispatchReportOptions = {
...
export async function exportDispatchReportToExcel({
  table,
  coldStorageName,
...
}: ExportDispatchReportOptions): Promise<void> {
  ...
  const worksheet = workbook.addWorksheet('Dispatch Gate Passes', {
  ...
  visibleColumns.forEach((column, columnIndex) => {
    ...
    if (isSummableExportColumn(columnId)) {
      ...
      const format = isIntegerSummableExportColumn(columnId) ? 'integer' : 'weight';
      cell.value = total;
      cell.style = createTotalStyle(align, getExcelNumFmt(format));
      return;
    }
  ...
  const filename = sanitizeFilename(
    `dispatch-gate-passes_${format(generatedAt, 'yyyy-MM-dd')}.xlsx`,
  );
