...
export async function getStorageGatePasses(
  ...
  try {
    const { data } = await apiClient.get<GetStorageGatePassesResponse>('/storage-gate-pass/', {
    ...
    throw new Error(getApiErrorMessage(error, 'Failed to load storage gate passes'), {
      cause: error,
    });
  }
}
