...
const INTEGER_COLUMNS = new Set<keyof TransferStockReportRow>(['totalBags']);
const WEIGHT_COLUMNS = new Set<keyof TransferStockReportRow>([]);
const NUMERIC_COLUMNS = new Set<keyof TransferStockReportRow>([
  ...INTEGER_COLUMNS,
...
export function getColumnExportLabel(
  column: Column<ReportFeatures, TransferStockReportRow, unknown>,
): string {
...
export function getExportCellForRow(
  row: Row<ReportFeatures, TransferStockReportRow>,
  column: Column<ReportFeatures, TransferStockReportRow, unknown>,
): ExportCellValue {
  ...
  const columnId = column.id as keyof TransferStockReportRow;
  const meta = column.columnDef.meta;
...
export function collectExportRows(
...
): Row<ReportFeatures, TransferStockReportRow>[] {
  ...
  function flattenGroupedRows(
    rows: Row<ReportFeatures, TransferStockReportRow>[],
  ): Row<ReportFeatures, TransferStockReportRow>[] {
    const result: Row<ReportFeatures, TransferStockReportRow>[] = [];
...
function formatColumnFilterSummary(table: Table<ReportFeatures, TransferStockReportRow>): string[] {
...
function formatAdvancedFilterSummary(
  ...
  if (manualSearch) {
    summaries.push(`Gate pass search: "${manualSearch}"`);
  }
...
function formatSortingSummary(table: Table<ReportFeatures, TransferStockReportRow>): string | null {
...
export function isNumericExportColumn(columnId: string): boolean {
  return NUMERIC_COLUMNS.has(columnId as keyof TransferStockReportRow);
}

export function isSummableExportColumn(columnId: string): columnId is keyof TransferStockReportRow {
  return (
    INTEGER_COLUMNS.has(columnId as keyof TransferStockReportRow) ||
    WEIGHT_COLUMNS.has(columnId as keyof TransferStockReportRow)
  );
}
