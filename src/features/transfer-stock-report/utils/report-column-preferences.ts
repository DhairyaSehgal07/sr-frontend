...
function toColumnState(
  ...
  columnIds: string[],
): TransferStockReportColumnState {
  if (!preferences) {
    return {
      columnVisibility: {},
      columnOrder: [],
...
export function getTransferStockReportColumnIds(
...
export function hasStoredTransferStockReportColumnState() {
...
export function clearStoredTransferStockReportColumnState() {
  const storage = getStorage();
  if (!storage) return false;
