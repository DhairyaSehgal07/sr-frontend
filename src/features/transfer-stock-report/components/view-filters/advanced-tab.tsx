...
import { isAdvancedNumericColumn } from '@/features/transfer-stock-report/utils/report-filter-fns';
...
function getColumnLabel(column: Column<ReportFeatures, TransferStockReportRow, unknown>) {
  return column.columnDef.meta?.filterLabel ?? column.id;
}

function getDefaultOperator(column: Column<ReportFeatures, TransferStockReportRow, unknown>) {
  return isAdvancedNumericColumn(column.id) ? 'greaterThan' : 'contains';
}

function getOperatorOptions(
  column: Column<ReportFeatures, TransferStockReportRow, unknown> | undefined,
) {
...
function createCondition(
  ...
  return {
    ...
    columnId: column.id as keyof TransferStockReportRow,
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Combine conditions with AND / OR logic. For example, variety equals Jyoti AND bags
              &gt; 10.
            </p>
          </div>
          <Button
