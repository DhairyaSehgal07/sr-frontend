...
const STORAGE_KEY = 'incoming-report:column-preferences:v1';
...
export type IncomingReportColumnState = {
...
export function getIncomingReportColumnIds(
  columns: ColumnDef<ReportFeatures, Record<string, unknown>>[],
) {
  return columns.map(getColumnId);
}

export function getStoredIncomingReportColumnState(columnIds: string[]): IncomingReportColumnState {
...
export function hasStoredIncomingReportColumnState() {
  return parsePreferences(getStorage()?.getItem(STORAGE_KEY) ?? null) != null;
}

export function saveIncomingReportColumnState(
  columnIds: string[],
  columnVisibility: ColumnVisibilityState,
  columnOrder: ColumnOrderState,
) {
...
export function clearStoredIncomingReportColumnState() {
  const storage = getStorage();
  if (!storage) return false;
