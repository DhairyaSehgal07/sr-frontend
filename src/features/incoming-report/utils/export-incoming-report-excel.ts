...
import { sumReportNumericColumn } from '@/features/incoming-report/utils/report-formatters';
...
export async function exportIncomingReportToExcel({
...
}: ExportIncomingReportOptions): Promise<void> {
  ...
  const worksheet = workbook.addWorksheet('Incoming Gate Passes', {
  ...
  visibleColumns.forEach((column, columnIndex) => {
    ...
    if (isSummableExportColumn(columnId)) {
      ...
      const format = columnId === 'bags' ? 'integer' : 'weight';
      cell.value = total;
  ...
  const filename = sanitizeFilename(
    `incoming-gate-passes_${format(generatedAt, 'yyyy-MM-dd')}.xlsx`,
  );
