  ...
  <body>
    ...
    <div class="toolbar">
      <button
        ...
        class="primary"
        onclick="downloadIncomingReportExcel()"
      >
        Download Excel
      </button>
    </div>
    <script>
      (function () {
        var DOWNLOAD_TYPE = ${JSON.stringify(INCOMING_REPORT_DOWNLOAD_EXCEL_MESSAGE)};
        var DONE_TYPE = ${JSON.stringify(INCOMING_REPORT_DOWNLOAD_EXCEL_DONE_MESSAGE)};
        ...
        window.downloadIncomingReportExcel = function () {
          if (!window.opener) {
