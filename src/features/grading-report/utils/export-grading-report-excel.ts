...
export async function exportGradingReportToExcel({
  table,
  coldStorageName,
  ...
  for (const line of exportLines) {
    ...
    if (!isGroupRow && line.incomingIndex === 0 && line.incomingCount > 1) {
      visibleColumns.forEach((column, columnIndex) => {
        ...
        mergeRanges.push({
          ...
          endRow: currentRowIndex + line.incomingCount - 1,
    ...
    if (!isGroupRow) {
      dataRowCounter += line.incomingIndex === 0 ? 1 : 0;
    }
  ...
  const filename = sanitizeFilename(`grading-report_${format(generatedAt, 'yyyy-MM-dd')}.xlsx`);
