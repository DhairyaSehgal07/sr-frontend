...
import {
  ...
  collectGradingExportLines,
  exportCellValueToDisplay,
  getColumnExportLabel,
  getExportCellForLine,
  getFilteredLeafRowCount,
  getFooterExportValue,
  isAverageExportColumn,
  isIncomingGatePassColumn,
...
function buildPreviewStyles(): string {
  ...
  return `
    ...
    * { box-sizing: border-box; }
    ...
    tbody td.incoming { background: color-mix(in srgb, var(--muted-fill) 45%, transparent); }
...
function renderCellHtml(
  ...
  return `<td class="${classNames}"${rowSpanAttr}>${escapeHtml(display)}</td>`;
}

export function buildGradingReportPreviewHtml({
  table,
  coldStorageName,
...
}: PreviewGradingReportOptions): string {
  const visibleColumns = table.getVisibleLeafColumns();
  const exportLines = collectGradingExportLines(table);
  ...
  const bodyRows = exportLines
    .map((line) => {
      const isGroupRow = line.row.getIsGrouped();
      const rowClass = isGroupRow ? 'group-row' : dataRowCounter % 2 === 1 ? '' : '';
      const zebraClass =
        !isGroupRow && dataRowCounter % 2 === 1 ? ' style="background: var(--zebra)"' : '';

      if (!isGroupRow && line.incomingIndex === 0) {
        dataRowCounter += 1;
      }

      const cells = visibleColumns
        .map((column) => {
          if (!isGroupRow && line.incomingIndex > 0 && !isIncomingGatePassColumn(column.id)) {
            return '';
          }

          const exportCell = getExportCellForLine(line, column);
          const rowSpan =
            !isGroupRow &&
            line.incomingIndex === 0 &&
            line.incomingCount > 1 &&
            !isIncomingGatePassColumn(column.id)
              ? line.incomingCount
              : undefined;

          return renderCellHtml(column, exportCell, {
            incoming: isIncomingGatePassColumn(column.id),
            rowSpan,
          });
        })
        .join('');

      return `<tr class="${rowClass}"${zebraClass}>${cells}</tr>`;
    })
    .join('');
  ...
  const footerCells = visibleColumns
    .map((column, columnIndex) => {
      ...
      if (isSummableExportColumn(columnId) || isAverageExportColumn(columnId)) {
        ...
        let display = exportCellValueToDisplay(exportCell);
  ...
  <body>
    ...
    <div class="toolbar">
      <button
        ...
        class="primary"
        onclick="downloadGradingReportExcel()"
      >
        Download Excel
      </button>
    </div>
    <script>
      (function () {
        var DOWNLOAD_TYPE = ${JSON.stringify(GRADING_REPORT_DOWNLOAD_EXCEL_MESSAGE)};
        var DONE_TYPE = ${JSON.stringify(GRADING_REPORT_DOWNLOAD_EXCEL_DONE_MESSAGE)};
        ...
        window.downloadGradingReportExcel = function () {
          if (!window.opener) {
...
export function openGradingReportPreview(options: PreviewGradingReportOptions): Window {
  const html = buildGradingReportPreviewHtml(options);
