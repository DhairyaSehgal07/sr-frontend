...
import type {
  ...
  GradingReportColumnId,
} from '@/features/grading-report/utils/report-filter-fns';
import { isAdvancedNumericColumn } from '@/features/grading-report/utils/report-filter-fns';
...
const NUMERIC_OPERATOR_OPTIONS: OperatorOption[] = [
  ...
  { value: 'greaterThanOrEqual', label: '>=', requiresValue: true },
  { value: 'lessThan', label: '<', requiresValue: true },
  { value: 'lessThanOrEqual', label: '<=', requiresValue: true },
  { value: 'isEmpty', label: 'is blank' },
  { value: 'isNotEmpty', label: 'is not blank' },
];
...
function createCondition(
  ...
  return {
    ...
    columnId: column.id as GradingReportColumnId,
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            ...
            <p className="text-muted-foreground text-sm">
              Combine conditions with AND / OR logic. For example, grading net &gt; 1,000 AND
              wastage % &lt; 5.
            </p>
          </div>
          <Button
