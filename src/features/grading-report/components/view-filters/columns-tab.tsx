...
import {
  clearStoredGradingReportColumnState,
  hasStoredGradingReportColumnState,
  saveGradingReportColumnState,
} from '@/features/grading-report/utils/report-column-preferences';
import { cn } from '@/lib/utils';

const COLUMN_ORDER_GROUP = 'grading-report-columns';
  ...
  return (
    ...
    >
      <button
        ref={handleRef}
        type="button"
        className="text-muted-foreground hover:bg-muted hover:text-foreground focus-visible:ring-ring/30 flex size-7 shrink-0 cursor-grab items-center justify-center rounded-md transition-colors active:cursor-grabbing focus-visible:ring-2 focus-visible:outline-none"
...
const ColumnsTab = <TData extends RowData>({
  ...
  const handleSaveDefaultColumns = () => {
    const saved = saveGradingReportColumnState(columnIds, draftColumnVisibility, draftColumnOrder);
  ...
  const handleClearDefaultColumns = () => {
    const cleared = clearStoredGradingReportColumnState();
