...
import { getStoredGradingReportColumnState } from '@/features/grading-report/utils/report-column-preferences';
import type { AdvancedReportGlobalFilter } from '@/features/grading-report/utils/report-filter-fns';
...
export function ViewFiltersSheet({ table }: ViewFiltersSheetProps) {
  ...
  const handleResetChanges = () => {
    ...
    const nextColumnState = getStoredGradingReportColumnState(
      table.getAllLeafColumns().map((column) => column.id),
    );
