...
import {
  clearStoredStorageReportColumnState,
  hasStoredStorageReportColumnState,
  saveStorageReportColumnState,
} from '@/features/storage-report/utils/report-column-preferences';
import { cn } from '@/lib/utils';

const COLUMN_ORDER_GROUP = 'storage-report-columns';
...
const ColumnsTab = <TData extends RowData>({
  ...
  const [hasSavedDefault, setHasSavedDefault] = useState(() => hasStoredStorageReportColumnState());
  ...
  const handleSaveDefaultColumns = () => {
    const saved = saveStorageReportColumnState(columnIds, draftColumnVisibility, draftColumnOrder);
  ...
  const handleClearDefaultColumns = () => {
    const cleared = clearStoredStorageReportColumnState();
