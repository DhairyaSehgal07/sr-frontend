...
import { writeStorageReportSummaryWorksheet } from '@/features/storage-report/utils/render-storage-report-summary-excel';
...
export async function exportStorageReportToExcel({
...
}: ExportStorageReportOptions): Promise<void> {
  ...
  writeStorageReportSummaryWorksheet(
    workbook,
    summaries,
    coldStorageName,
    quantityMode === 'current' ? 'Current Qty' : 'Initial Qty',
  );

  const buffer = await workbook.xlsx.writeBuffer();
  const filename = sanitizeFilename(`storage-report_${format(generatedAt, 'yyyy-MM-dd')}.xlsx`);
