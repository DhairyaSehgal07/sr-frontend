...
const STORAGE_KEY = 'storage-report:column-preferences:v1';
...
export type StorageReportColumnState = {
...
export function getStorageReportColumnIds(
  columns: ColumnDef<ReportFeatures, Record<string, unknown>>[],
) {
  return columns.map(getColumnId);
}

export function getStoredStorageReportColumnState(columnIds: string[]): StorageReportColumnState {
...
export function hasStoredStorageReportColumnState() {
  return parsePreferences(getStorage()?.getItem(STORAGE_KEY) ?? null) != null;
}

export function saveStorageReportColumnState(
  columnIds: string[],
  columnVisibility: ColumnVisibilityState,
  columnOrder: ColumnOrderState,
) {
...
export function clearStoredStorageReportColumnState() {
  const storage = getStorage();
  if (!storage) return false;
