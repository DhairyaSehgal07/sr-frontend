    ...
    <div className="flex w-full flex-col gap-4">
      ...
      <div className="space-y-6">
        {Array.from({ length: 3 }).map((_, index) => (
          <BookingGatePassCardSkeleton key={index} />
        ))}
      </div>
  ...
  return (
    <div className="flex w-full flex-col gap-4">
      <Item variant="outline" size="sm">
        ...
        <ItemContent>
          <ItemTitle>{totalCount} booking gate passes</ItemTitle>
        </ItemContent>
      ...
      {showSearchLoading ? (
        ...
        <Empty className="rounded-xl border bg-muted/10">
          <EmptyHeader>
            ...
            <EmptyTitle>Could not load booking gate passes</EmptyTitle>
        ...
        <div className="space-y-6">
          {bookings.map((booking) => (
            <BookingGatePassCard key={booking._id} data={booking} />
          ))}
        </div>
      ) : (
        <Empty className="rounded-xl border bg-muted/10">
