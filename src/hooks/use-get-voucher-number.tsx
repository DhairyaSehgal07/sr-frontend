...
export const voucherNumberKeys = {
  ...
  detail: (type: VoucherNumberType) => [...voucherNumberKeys.all, 'detail', type] as const,
};
...
async function fetchVoucherNumber(type: VoucherNumberType = 'incoming-gate-pass'): Promise<number> {
  try {
    const { data } = await apiClient.get<VoucherNumberApiResponse>('/store-admin/voucher-number', {
      params: { type },
    });
...
export const voucherNumberQueryOptions = (type: VoucherNumberType = 'incoming-gate-pass') =>
  queryOptions({
...
export function useGetReceiptVoucherNumber(type: VoucherNumberType = 'incoming-gate-pass') {
  return useQuery({
...
export function prefetchVoucherNumber(type: VoucherNumberType = 'incoming-gate-pass') {
